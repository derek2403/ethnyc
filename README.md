...
### The headline flow — negotiate → audit → verify
1. **Negotiate (HCS-16 chat room).** The requester posts an ask; the **auditor's quote is generated by OpenAI** (auditor only — `lib/auditor.ts`, default `gpt-5.4-nano-2026-03-17`, tightly constrained, pinned to a **1 USDC** fee); the requester accepts. Every line is a real HCS-16 message in one global room (seeded to `mars-state.json`).
...
3. **Audit (real, 4 OpenAI stages).** `lib/audit-core.mjs` runs **Scanner → Sandbox → Fork → Synthesizer** over the actual skill source. Each stage is recorded on the task topic (summary + findings + severity histogram + model); the verdict carries `risk`, a 0–100 trust score, **`capabilities` ("what the skill actually does")**, and a recommendation; the full untruncated report is stored on **HCS-1**. *(No `OPENAI_API_KEY` → deterministic fallback so the demo never breaks.)*
4. **Decide → review → mint.** The requester **approves** (SAFE) or **disapproves/blocks** (DANGEROUS); rates + reviews the auditor (written to the auditor's own **review** + **HCS-20 voting** topics → reputation); and for an approved SAFE skill a **VERIFIED HTS NFT** is minted and transferred to the requester. Each step is a row on the task topic, and the job is `job_updated` in the main registry (→ the verified-skills dashboard).

The task topic is the full replayable record: `init → 3 stage messages → verdict → decision → reviewed → minted`.
