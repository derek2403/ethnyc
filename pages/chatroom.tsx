...
import { REQUESTER, AUDITOR, SKILLS, negoScript, type StepStatus } from "@/lib/demo-skills";
...
export default function ChatRoom() {
  ...
  async function runNego() {
    ...
    for (const turn of negoScript(skill)) {
      ...
      setPending((p) => [...p, { id, from: turn.from, text: turn.text }]);
      await call({ action: "floraChat", commsTopicId: room.chatRoomTopicId, floraAccountId: room.floraId, senderId: turn.from, data: turn.text });
      await sleep(1500);
    }
    setNego("agreed");
  }
