...
// Resolves the requester account, then runs the full lifecycle: negotiate + create
// task → real audit → add to db + /skills + whitelist → requester approval mints a
// VERIFIED NFT. The flow lives in lib/task-flow.ts so the curl endpoint
// (pages/api/run-task.ts) runs the exact same code.
//
