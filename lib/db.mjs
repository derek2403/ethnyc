...
export function deriveState() {
  ...
  for (const id of [...new Set(recs.map((r) => r.agent_id).filter(Boolean))]) {
    ...
    userMap[id] = { id, worldId: "—", skills: licensed(id), spend: 0, since: "—", rating: 0, sessions: sessions(id), last: recs.find((r) => r.agent_id === id)?.skill || "—", active: recs.some((r) => r.agent_id === id && String(r.status).startsWith("auditing")) };
  }
