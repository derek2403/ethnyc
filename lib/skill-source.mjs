...
export async function resolveRemoteSkill(skill) {
  ...
  if (/^https?:\/\//i.test(s)) {
    // GitHub "blob" page URLs aren't raw text — rewrite to raw.githubusercontent.com.
    const gh = s.match(/^https?:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/(.+)$/i);
    const url = gh ? `https://raw.githubusercontent.com/${gh[1]}/${gh[2]}/${gh[3]}` : s;
    const content = await fetchText(url);
    const file = basename(new URL(url).pathname) || "skill.txt";
    return { name: file.replace(/\.[^.]+$/i, "") || "skill", files: [{ name: file, content }] };
  }
  return resolveNpm(s);
}
